...
class TestCurrency(unittest.TestCase):
    ...
    def test_calculate_conversion_negative_amount(self, mock_get_rate):
        """EDGE CASE: Negative amount should work mathematically."""
    ...
    @patch('Currency.get_exchange_rate')
    def test_calculate_conversion_zero(self, mock_get_rate):
        """EDGE CASE: Converting 0."""
        mock_get_rate.return_value = {"success": True, "rate": 25000.0}

        result = Currency.calculate_conversion(0, "USD", '1')

        self.assertTrue(result['success'])
        self.assertEqual(result['converted_string'], "0 VND")

    @patch('Currency.get_exchange_rate')  # <--- FIXED: Added Mock
    def test_calculate_conversion_invalid_type(self, mock_get_rate):  # <--- FIXED: Added Argument
        """EDGE CASE: Non-numeric input (e.g., string 'abc')."""

        # 1. Force API success so logic proceeds to math
        mock_get_rate.return_value = {"success": True, "rate": 25000.0}

        # 2. Expect TypeError because "abc" * 25000.0 is invalid in Python
        with self.assertRaises(TypeError):
            Currency.calculate_conversion("abc", "USD", '1')
